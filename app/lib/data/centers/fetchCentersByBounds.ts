...
export async function fetchCentersByBounds(
  ...
  try {
    ...
    const data = await sql<Center>`
      WITH matched_centers AS (
        ...
        WHERE 
          ...
          g.latitude BETWEEN ${bounds.south} AND ${bounds.north} AND
          g.longitude BETWEEN ${bounds.west} AND ${bounds.east}
        GROUP BY g.id
        HAVING 
    ...
    if (error instanceof Error) {
      console.error(
        "Error occurred while fetching centers by bounds:",
        error.message
      );
      throw new Error(`Failed to fetch centers by bounds: ${error.message}`);
    } else {
      console.error("An unknown error occurred:", error);
      throw new Error("Failed to fetch centers by bounds: Unknown error");
    }
  }
}
